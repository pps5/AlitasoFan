...
firebase.auth().onAuthStateChanged(user => {
    if (user == null) {
        ...
        db_1.FirebaseDB.fetchOriginals().then(originals => {
            var options = '';
...
document.getElementById('submit').addEventListener('click', e => {
    ...
    uploadImage(uploadFile, uploadFileName).then(() => {
        ...
        db_1.FirebaseDB.writeImageData(uploadFileName, character, original, sellingPoint).then(result => {
            document.getElementById('new-pic-form').reset();
            document.getElementById('preview-area').className = 'hide';
        });
        db_1.FirebaseDB.writeNewOriginal(original);
        toggleFormEnabled();
    }, error => {
        console.log(error);
